...
use `OptionWrapper` to render components around each child.
...
<Playground>
  ...
  <Button.Group
    ...
    OptionWrapper={({children}) => <View m={4}>{children}</View>}>
...
<Playground>
  ...
  <Button.Group
    ...
    onChange={(values) => console.log('selected', values)}
    strategy={{
      isSelected: (selectedValue, value) => console.log(1, selectedValue, value) || selectedValue === value,
      update: (selectedValue, value) => console.log(2, selectedValue, value) || (selectedValue === value ? undefined : value)
    }}>
    <Button value="a">Button A</Button>
    <Button value="b">Button B</Button>
    <Button value="c">Button C</Button>
